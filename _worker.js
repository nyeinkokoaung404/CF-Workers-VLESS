...
<body>
    <div class="login-container">
        ...
        <div class="footer">
            <p>Powered by eooce <a href="https://t.me/premium_channel_404" target="_blank" style="color: #007bff; text-decoration: none;">Join Telegram Channel</a></p>
        </div>
    </div>
...
/**
 * Retrieve homepage content (displayed after successful password verification)
 * @param {string} url 
 * @param {string} baseUrl 
 * @returns {Response}
 */
...
<body>
    ...
    <div class="container">
        <div class="logo"><img src="https://img.icons8.com/color/96/cloudflare.png" alt="Logo"></div>
        <h1 class="title">Workers Service</h1>
        <p class="subtitle">High-performance networking service based on Cloudflare Workers (VLESS + Trojan)</p>
        ...
        <div class="button-group">
            <button onclick="copyNodeSubscription()" class="btn btn-secondary">Copy Node Link</button>
            <button onclick="copySingboxSubscription()" class="btn btn-secondary">Copy Singbox Subscription Link</button>
            <button onclick="copyClashSubscription()" class="btn btn-secondary">Copy Clash Subscription Link</button>
            <button onclick="copySubscription()" class="btn btn-secondary">Copy V2rayN Subscription Link</button>
        </div>
        
        <div class="footer">
            <div class="footer-links">
                <a href="https://github.com/nyeinkokoaung404/CF-Workers-VLESS" target="_blank" class="footer-link">
                    ...
                    <span>GitHub Project Address</span>
                </a>
                <a href="https://check-proxyip.ssss.nyc.mn/" target="_blank" class="footer-link">
                    <span>✅</span>
                    <span>ProxyIP Detection Service</span>
                </a>
                <a href="https://t.me/premium_channel_404" target="_blank" class="footer-link">
                    <span>📱</span>
                    <span>Telegram Feedback and Discussion Group</span>
                </a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function copySubscription() {
            ...
            navigator.clipboard.writeText(configUrl).then(() => {
                showToast('V2rayN subscription link has been copied to your clipboard!');
            }).catch(() => {
                const textArea = document.createElement('textarea');
                textArea.value = configUrl;
                document.body.appendChild(textArea);
        ...
        function copyClashSubscription() {
            ...
            navigator.clipboard.writeText(clashUrl).then(() => {
                showToast('Clash subscription link has been copied to your clipboard!');
            }).catch(() => {
                const textArea = document.createElement('textarea');
        ...
        function copySingboxSubscription() {
            ...
            navigator.clipboard.writeText(singboxUrl).then(() => {
                showToast('Singbox subscription link has been copied to your clipboard!');
            }).catch(() => {
                const textArea = document.createElement('textarea');
